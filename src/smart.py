...
class Player:
        ...
        if max(len(self.myList), len(self.opponentList)) <= self.ESTIMATION_THRESHOLD:
            ...
            try:
                for maxDepth in range(self.FIRST_DEPTH, self.MAX_DEPTH):
                    ...
                    newHand = self.evaluate(myHand, opponentHand, lastAction, self.MIN_UTILITY, self.MAX_UTILITY, 0, firstCall=True)
            except TimeoutError:
                pass
    ...
    def evaluate(self, myHand, opponentHand, lastAction, alpha, beta, depth, firstCall=False):
        """
